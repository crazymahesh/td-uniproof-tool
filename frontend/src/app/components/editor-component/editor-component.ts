...
export class EditorComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
  ...
  private currentAuthor = 'Unknown Author';
  ...
  constructor(
    ...
    this.userService.user$.subscribe(user => {
      this.currentAuthor = user?.['username'] || user?.['name'] || user?.['email'] || 'Unknown Author';
    });
  }
  ...
  ngOnInit(): void {
    ...
    this.editor = new Editor({
      ...
      plugins: [
        ...
        trackChangesPlugin(this.schema, this.trackChangesService, () => this.currentAuthor),
      ],
    });
  }
  ...
  navigateToPosition(from: number, to: number): void {
    if (!this.editor?.view) return;
    const { state, dispatch } = this.editor.view;
    const docSize = state.doc.content.size;
    const clampedFrom = Math.max(1, Math.min(from, docSize - 1));
    const clampedTo   = Math.max(clampedFrom, Math.min(to, docSize - 1));
    try {
      const sel = TextSelection.create(state.doc, clampedFrom, clampedTo);
      dispatch(state.tr.setSelection(sel).scrollIntoView());
      this.editor.view.focus();

      // ProseMirror's scrollIntoView targets the editor's own viewport; the
      // page uses a custom scroll container (.editor-main), so we also need
      // a DOM-level scroll after Angular has flushed the selection.
      setTimeout(() => {
        try {
          const view = this.editor.view;
          if (!view) return;
          const coords = view.coordsAtPos(clampedFrom);
          const scrollEl = document.querySelector<HTMLElement>('.editor-main');
          if (scrollEl && coords) {
            const containerRect = scrollEl.getBoundingClientRect();
            // coords.top is in viewport px; convert to offset within the scroll container
            const targetOffset = scrollEl.scrollTop + (coords.top - containerRect.top);
            scrollEl.scrollTo({ top: targetOffset - scrollEl.clientHeight / 3, behavior: 'smooth' });
          }
        } catch { /* doc may have changed */ }
      }, 30);
    } catch {
      // position may be out of range if the doc changed
    }
  }
