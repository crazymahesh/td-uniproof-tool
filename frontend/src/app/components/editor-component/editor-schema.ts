...
export const tableSchema = new Schema({
  ...
  marks: ngxSchema.spec.marks.append({
    insertion: {
      attrs: { id: { default: null }, author: { default: '' }, timestamp: { default: '' }, groupId: { default: null } },
      parseDOM: [{
        tag: 'span.insertion',
        getAttrs: (dom: any) => ({
          ...
          groupId: dom.getAttribute('data-group-id') || null,
        }),
      }],
      toDOM(mark) {
        ...
        if (mark.attrs['groupId']) attrs['data-group-id'] = mark.attrs['groupId'];
        return ['span', attrs, 0];
      },
    },
    deletion: {
