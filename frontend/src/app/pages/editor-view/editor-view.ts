...
export class EditorView implements OnInit, OnDestroy {
  ...
  // Track Changes (enabled by default — mirrors TrackChangesService initial state)
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(
      this.trackChangesService.enabled$.subscribe(enabled => {
        this.trackChangesEnabled = enabled;
      })
    );
  ...
  toggleTrackChanges(): void {
    this.trackChangesService.toggle();
  }

  navigateToChange(change: TrackChange): void {
    if (this.editorComponent) {
      this.editorComponent.navigateToPosition(change.from, change.to);
    }
  }

  getChangeIcon(type: TrackChange['type']): string {
    if (type === 'insertion') return 'fas fa-plus-circle';
    if (type === 'deletion') return 'fas fa-minus-circle';
    return 'fas fa-exchange-alt';
  }

  getChangeLabel(change: TrackChange): string {
    if (change.nodeType === 'figure') return '[Figure inserted]';
    if (change.nodeType === 'table') return '[Table inserted]';
    if (change.type === 'modification') return `"${change.oldContent}" → "${change.content}"`;
